...
> **정정(2026-09-10) — 이 두 칸은 원시 파일이 없다.** `probe_path_naming.py` 는
> 결과를 `print` 만 하고 회차 파일을 안 남긴다. 저장소에도 git 이력에도 `0.233` ·
> `1.000` 을 담은 파일이 없어서 **회차 단위로 다시 셀 수단이 없다.** 방향(경로가
> 실행률을 지배한다)은 같은 축의 다른 값들과 함께 서지만, 이 두 분수는 **재측정
> 대기**다. 값은 지우지 않는다 — 이 표가 무엇을 근거로 두 절을 철회했는지가 기록의
> 일부다. 인용할 때 이 상자를 같이 옮겨라.
...
> **2026-09-10 정정.** 위 네 칸(`29/82` · `18/18` · `30/30`/`10/10` · `20/21`)과
> 세 p 는 **원시 파일이 없다.** `modelaxis-*.json` · `pathnaming-*.json` 이
> 디스크에도 git 이력에도 0 건이다. 그래서 `README.md` 3절에서 **검증 가능한
> 원본 부재로 제외**됐고, 그에 기대던 결론(모델축 상호작용 · 모델 층 판정)도
> 같이 내려갔다. 이 줄은 **그날 그렇게 판단했다는 기록으로만** 남긴다 —
> 현행 주장으로 인용하지 마라. 추적 사슬과 재측정 명령은 `remeasure.yaml` 의
> `pathcue-*` 일곱 항목에 있다.
>
> `18/18` 은 원본 부재에 더해 **출처까지 어긋난다** — 유일한 출처인 봉인 문서
> `docs/90` 의 「통합 표본」 표가 같은 문서 2.5 항(`haiku` 10/10 · `opus` 10/10,
> 합 20/20)과 맞지 않는다. 통합이 분모를 20 에서 18 로 줄일 수는 없다.
...
> **정정(감사).** B 의 `.RRR` 을 10/12 에서 **9/12** 로 고쳤다. 같은 표의
> 전부-같음 2 · 턴1 2/12 · 턴2-4 35/36 과 양립하지 않는다 — 턴2-4 의 실패 한 번이
> 턴1 이 R 인 세션에 있으면 전부-같음이 1 이 되어 버린다. 전수 열거로 남는 배치는
> `.RRR` 9 뿐이다. 이 축은 원시 파일이 없어 표끼리 맞추는 것 말고 확인 수단이 없다.
...
**샌드박스 팔 둘은 구분되지 않는다**(`p = 1.000`). 기저율은 안 변했고 바뀐 것은
...
> **정정(감사).** 여기 "통계적으로 같다" 를 **"구분되지 않는다"** 로 고쳤다 —
> 차이 95% CI 가 [−0.17, +0.13] 이라 동등성은 주장할 수 없다. 그리고 위 `Agent × 2`
> 는 **회차 둘이 아니다**: 두 항목의 워크스페이스 경로가 같아 **한 회차**이고
> `sub_got` 도 1 이다. 채널 저장이 이름당 5 개 캡이므로 이 둘이 전부다. 즉 남은
> `6/59` 중 경로가 확인된 것은 **한 회차**이고, 나머지 다섯은 어느 호출로 나왔는지
> 원시에 없다. 문서의 "6/59 는 위임" 은 그만큼 낮춰 적었다.
...
## 게시된 p 49개를 기계가 읽는 목록으로 모았다 — `p-family.yaml`

**상태** 완료(오프라인) · **유료 회차** 0

### 한 일

`<!-- p: ... -->` 근거 주석 하나가 대비 하나라는 규율은 이미 있었는데, 그 대비가
**무엇을 반증하려 했고 어떤 원시가 지탱하는지**는 어디에도 모여 있지 않았다.
문서를 처음부터 읽어야 알 수 있었고, 그래서 아무도 세지 않았다.

`p-family.yaml` 에 49개를 모았다. 항목마다 `test_id` · 가설 · 결과 변수 ·
비교 조건 · 원본 데이터 · 쓰인 결론 · 가족이 붙는다.

### 원본을 어디에 두었나

**대비의 존재와 인쇄된 p 는 여전히 게시 문서의 근거 주석이 원본이다.**
`check_docs.py` 의 `family()` 가 거기서 인벤토리를 유도하고, 새 파일은 그 주석을
**키로 받아** 한 줄 주석에 못 담는 것만 붙인다. 두 벌을 만들지 않으려는 것이다 —
이 저장소가 같은 값을 두 군데 적어서 갈린 적이 여러 번 있다. `p_printed` 와
`corrected` 도 손으로 안 적었다. `family()` 가 읽은 인쇄값과 `multiplicity()` 의
임계에서 유도했다. 두 집합이 어긋나면 검사가 실패해야 하고, 그 검사는 다음 단계로
넘겼다.

### 가족을 다시 갈랐지만 보정은 안 건드렸다

가족을 **연구 질문과 선택·비교 범위**로 갈랐다(문서 위치가 아니다). 49개가 18개
가족에 들어간다 — 같은 7절 안에서도 방식·모델·픽스처는 다른 질문이고, 네 문서에
재진술된 같은 대비는 한 번만 센다.

**구속력 있는 보정은 그대로 저장소 전역 N=49 다.** 질문별로 쪼개면 N 이 작아져
임계가 느슨해진다 — 표기를 고치는 대신 검사를 무르게 하는 쪽이라 안 한다. 쪼갠
단위로 보정을 다시 걸지는 사람이 정할 설계 변경이다.

### 세면서 나온 것

**원시가 지탱하는 대비는 49개 중 33개다.** 나머지 16개는 인쇄된 2x2 를 Fisher 에
다시 넣는 것까지이고(`recompute: 표기`), 그중 둘은 그것조차 안 된다 —
세션 상관 이항검정 둘은 2x2 가 없어 `check_p` 가 미검증으로 인쇄한다.
**근거가 없는 값을 "재계산 가능" 으로 적지 않으려고 세 갈래로 나눴다.**

원시가 없는 자리는 이미 알려진 셋에 몰려 있다 — 경로/모델 축(콘솔만 냈다),
세션 축(같은 이유), 7절 앞 절반(고정 파일명 덮어쓰기, `remeasure.yaml`
`names-leak-rate`). 그리고 **등록부에 아직 안 올라간 것**이 있다: 경로 접두사 축과
모델 2x3, `allow-override.out` 두 판이 그렇다.

### 같이 한 것

**`docs/90` 을 봉인으로 표시했다.** 수치는 한 글자도 안 고치고, 머리 상자에 봉인
선언과 현행 문서 링크표를 붙였다. 2.6·2.7 두 절은 원시가 없다는 표시를 표 바로
위에 달았다 — `classify_executed.py` 는 지금도 화면에만 내고,
`classify_refusals.py` 는 파일을 남기게 고쳐졌지만 그 뒤로 안 돌았다(파일 0건).
**봉인은 인용 금지이지 검사 면제가 아니다.** `README.md` 3절이 그 절의 값들
(`silent` 8/8 · 언급률 8/8 · 0.488)을 현행 주장에 쓰고 있으므로, 그 자리는 검사
범위에 남아야 한다 — 다음 단계로 넘겼다.

**`probe_network.py` 가 팔의 프로토콜을 결과에 적는다.** 팔마다 스킴이 다른데
(우리 리스너는 평문 `http`, 밖 도메인은 `https`) 파일에는 `host` 만 있었다.
`scheme` 과 `url_tmpl` 을 팔 기록에 넣었다. **측정 설계는 안 바꿨다** — 스킴 차이는
관측 지점의 성질이고, 두 줄을 한 실험으로 만드는 것은 `remeasure.yaml` 의
`proxy-axis-2x2` 이지 이 커밋이 아니다. 순서(`order: block`)는 이미 적히고 있었고
난수 씨앗은 이 프로브에 없다(카나리는 `secrets` 로 뽑고 회차마다 새로 만든다).

### 안 한 것

회차를 하나도 안 태웠다. 확인은 전부 오프라인이다 — `check_docs` 의 `family()` ·
`multiplicity()` 를 읽기로 부르고, 원시 파일을 파싱해 분수를 대조하고,
`probe_network.arm()` 은 `STOPPED` 가 세워진 상태로 불러 모델 호출 없이 기록
필드만 봤다.
